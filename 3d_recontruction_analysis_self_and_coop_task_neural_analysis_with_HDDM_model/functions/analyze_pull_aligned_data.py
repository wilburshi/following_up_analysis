...
def analyze_pull_aligned_data(pull1_ts, pull2_ts, gaze1_ts, gaze2_ts, speed1_ts, speed2_ts, resolution_s,  time_window_start_s, time_window_end_s, coop_window_s):
    """
    ...
    # Fix: Define coop_window_steps within this function scope
    ...
    # A pull is 'successful' if it occurs within coop_window_s of the *other* animal's pull
    # And specifically, the REWARD happens if *both* pull within the window.
    # For 'time since last reward', we need the times of the *joint* successes.
    
    # Create a unified list of all pull events with animal ID
    all_pull_events = []
    ...
    processed_indices_for_reward = set() # To avoid double counting same cooperative event

    for i, p1_event in enumerate(all_pull_events):
        if p1_event['animal'] == 1 and p1_event['time_idx'] not in processed_indices_for_reward:
            # Check for a partner pull (animal 2) within coop window
            for j in range(i + 1, len(all_pull_events)):
                p2_event = all_pull_events[j]
                if p2_event['animal'] == 2 and abs(p1_event['time_idx'] - p2_event['time_idx']) <= coop_window_steps:
                    reward_times_indices.append(max(p1_event['time_idx'], p2_event['time_idx'])) # Reward at later pull
                    processed_indices_for_reward.add(p1_event['time_idx'])
                    processed_indices_for_reward.add(p2_event['time_idx'])
                    break # Found cooperative partner for this p1 pull

    reward_times_indices.sort() # Ensure rewards are sorted chronologically
    ...
    a1_prev_trial_outcome = [] # 1 for success, 0 for failure

    last_reward_time_a1_context = 0.0 # Context for calculating time since last reward
    failed_count_a1_context = 0
    prev_pull_time_a1 = 0.0 # For calculating RT for self-pull
    prev_outcome_a1 = 0 # Default to failed for the first pull

    for i, p_idx in enumerate(a1_pull_indices):
        current_pull_time_s = p_idx * resolution_s

        # Calculate RT (time since previous self-pull)
        rt = current_pull_time_s - prev_pull_time_a1
        if i == 0: # Handle first pull
            rt = np.nan # No previous pull for the very first one, or set to a sensible value like 0.1s
            
        # Gaze AUC and Partner and Self Speed Mean
        self_gaze_segment = get_aligned_segment(gaze1_ts, p_idx, window_steps_pre_for_helper, window_steps_post_for_helper, total_analysis_window_steps, num_time_points)
        gaze_auc = np.nansum(self_gaze_segment) * resolution_s
        a1_gaze_aucs.append(gaze_auc)

        partner_speed_segment = get_aligned_segment(speed2_ts, p_idx, window_steps_pre_for_helper, window_steps_post_for_helper, total_analysis_window_steps, num_time_points)
        partner_speed_mean = np.nanmean(partner_speed_segment)
        a2_speed_means_for_a1_pulls.append(partner_speed_mean)

        self_speed_segment = get_aligned_segment(speed1_ts, p_idx, window_steps_pre_for_helper, window_steps_post_for_helper, total_analysis_window_steps, num_time_points)
        ...
        for rw_idx in reward_times_indices:
            if abs(p_idx - rw_idx) <= coop_window_steps: # Check if this A1 pull is near a recorded reward
                current_pull_is_rewarded = True
                break
        
        # Store context variables for the current trial
        ...
        a1_prev_trial_outcome.append(prev_outcome_a1)

        # Update context for the *next* trial
        prev_pull_time_a1 = current_pull_time_s
        if current_pull_is_rewarded:
            failed_count_a1_context = 0
            last_reward_time_a1_context = current_pull_time_s # Update reward time to current pull time
            prev_outcome_a1 = 1 # Mark current pull as successful for next trial's 'prev_trial_outcome'
        else:
            failed_count_a1_context += 1
            prev_outcome_a1 = 0 # Mark current pull as failed


        # Add to HDDM DataFrame list for Animal 1
        if not np.isnan(rt): # Only include trials with a valid RT
            hddm_data_a1.append({
                'subj_idx': 'animal1',
                'rt': rt,
                'response': 1, # Always 1 since we're modeling pull events
                'self_gaze_auc': gaze_auc,
                ...
                'prev_trial_outcome': prev_outcome_a1 # Note: This is the outcome of the *current* pull for the *next* trial
            })

    # Adjust prev_trial_outcome for hddm_data_a1 to be truly 'previous'
    # The 'prev_trial_outcome' stored is the outcome of the *current* pull.
    # For the DDM, we want the outcome of the pull *before* the RT was measured.
    # So, shift it by one, and handle the first entry.
    if len(hddm_data_a1) > 0:
        actual_prev_outcomes_a1 = [0] + [d['prev_trial_outcome'] for d in hddm_data_a1[:-1]]
        for i, d in enumerate(hddm_data_a1):
            d['prev_trial_outcome'] = actual_prev_outcomes_a1[i]
        hddm_data_a1[0]['prev_trial_outcome'] = 0 # First pull has no previous outcome, default to 0 (failure)


    # --- Metrics for Animal 2's Pulls ---
    a2_pull_indices = np.where(pull2_ts == 1)[0]
    a2_pull_times_s = a2_pull_indices * resolution_s
    ...
    for i, p_idx in enumerate(a2_pull_indices):
        ...
        rt = current_pull_time_s - prev_pull_time_a2
        if i == 0:
            rt = np.nan

        self_gaze_segment = get_aligned_segment(gaze2_ts, p_idx, window_steps_pre_for_helper, window_steps_post_for_helper, total_analysis_window_steps, num_time_points)
        gaze_auc = np.nansum(self_gaze_segment) * resolution_s
        a2_gaze_aucs.append(gaze_auc)

        partner_speed_segment = get_aligned_segment(speed1_ts, p_idx, window_steps_pre_for_helper, window_steps_post_for_helper, total_analysis_window_steps, num_time_points)
        partner_speed_mean = np.nanmean(partner_speed_segment)
        a1_speed_means_for_a2_pulls.append(partner_speed_mean)

        self_speed_segment = get_aligned_segment(speed2_ts, p_idx, window_steps_pre_for_helper, window_steps_post_for_helper, total_analysis_window_steps, num_time_points)
        self_speed_mean = np.nanmean(self_speed_segment)
        ...
        a2_prev_trial_outcome.append(prev_outcome_a2)
        ...
        if not np.isnan(rt):
            hddm_data_a2.append({
                ...
                'prev_trial_outcome': prev_outcome_a2
            })

    if len(hddm_data_a2) > 0:
        actual_prev_outcomes_a2 = [0] + [d['prev_trial_outcome'] for d in hddm_data_a2[:-1]]
        for i, d in enumerate(hddm_data_a2):
            d['prev_trial_outcome'] = actual_prev_outcomes_a2[i]
        hddm_data_a2[0]['prev_trial_outcome'] = 0
    ...
    a1_prev_trial_outcome_filtered = [item['prev_trial_outcome'] for item in hddm_data_a1 if not np.isnan(item['rt'])] # This will be previous outcome for current trial
    ...
    if len(a1_gaze_aucs_filtered) > 1 and len(a2_speed_means_for_a1_pulls_filtered) > 1: # Need at least 2 points for correlation
    ...
    # New correlations: Gaze AUC vs. Failed Pulls Before Reward
    ...
    # New correlations: Gaze AUC vs. Time Since Last Reward
