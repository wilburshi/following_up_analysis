...
def continuous_variable_glm(KERNEL_DURATION_S, N_BASIS_FUNCS, fps, animal1, animal2, session_start_time, time_point_pull1, time_point_pull2, oneway_gaze1, oneway_gaze2, mutual_gaze1, mutual_gaze2, animalnames_videotrack, output_look_ornot, output_allvectors, output_allangles, output_key_locations):
    ...
    for ianimal in np.arange(0,nanimals,1):
        ...
        # Compute gaze_auc per pull (and convert to full-length regressor)
        if 0:
            window_len = int(KERNEL_DURATION_S * fps)
            gaze_auc_vector = np.zeros(len(Y))

            for pf in pull_idx:
                if pf - window_len >= 0:
                    auc = np.trapz(socialgaze_prob[pf - window_len:pf])
                    gaze_auc_vector[pf] = auc

            # Final Design Matrix
            X_full = np.hstack([X_continuous, gaze_auc_vector.reshape(-1, 1)])
        else:
            X_full = X_continuous
        ...
        logit_result = logit_model.fit()  # or fit_regularized() if still unstable
        # print(logit_result.summary())

        #
        # Save summary
        # summary_df.to_csv("glm_logit_summary.csv", index=False)
        var_labels = []
        for var in var_names:
            var_labels += [f"{var}_b{i+1}" for i in range(N_BASIS_FUNCS)]
        # var_labels.append('gaze_auc')

        # Extract Coefficients & p-values
        # Build summary DataFrame
        summary_df = logit_result.summary2().tables[1].copy()
        ...
        summary_df['Variable'] = ['Intercept'] + list(var_labels)  # Add names

        #
        # === Perform LRT for each variable ===

        lrt_results = []

        for i, var in enumerate(var_names):
            start_idx = 1 + i * N_BASIS_FUNCS  # +1 to skip intercept column at 0
            end_idx = start_idx + N_BASIS_FUNCS

            # Remove columns for this variable
            X_reduced = np.delete(X_design, np.s_[start_idx:end_idx], axis=1)

            # Fit reduced model
            reduced_model = sm.Logit(Y, X_reduced).fit(disp=0)

            # Calculate LRT stat and p-value
            LR_stat = 2 * (logit_result.llf - reduced_model.llf)
            p_val = chi2.sf(LR_stat, df=N_BASIS_FUNCS)

            lrt_results.append({'Variable': var, 'LRT_stat': LR_stat, 'p_value': p_val})
        ...
        mean_betas = []
        for i, var in enumerate(var_names):
            start_idx = 1 + i * N_BASIS_FUNCS
            end_idx = start_idx + N_BASIS_FUNCS
            mean_beta = summary_df.iloc[start_idx:end_idx]['Coefficient'].mean()
            mean_betas.append(mean_beta)

        mean_beta_df = pd.DataFrame({
            'Variable': var_names,
            'MeanBeta': mean_betas,
            'LRT_pvalue': lrt_df['p_value']
        })

        # save data into the summary data set
        if ianimal == 0:
            glm_fitting_summary[(animal1,'summary_df')] = summary_df
            glm_fitting_summary[(animal1,'lrt_df')] = lrt_df
            glm_fitting_summary[(animal1,'mean_beta_df')] = mean_beta_df
        elif ianimal == 1:
            glm_fitting_summary[(animal2,'summary_df')] = summary_df
            glm_fitting_summary[(animal2,'lrt_df')] = lrt_df
            glm_fitting_summary[(animal2,'mean_beta_df')] = mean_beta_df
