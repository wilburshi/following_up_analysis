...
#
def make_gaussian_basis(duration_s, n_basis, dt, sigma_scale=0.5):
    t = np.arange(0, duration_s, dt)
    centers = np.linspace(0, duration_s, n_basis)
...
# use the key short list of the variable and do the 80 20 prediction, save the prediction results

def continuous_variable_glm_shortlist_prediction(KERNEL_DURATION_S, N_BASIS_FUNCS, fps, animal1, animal2, session_start_time, time_point_pull1, time_point_pull2, oneway_gaze1, oneway_gaze2, mutual_gaze1, mutual_gaze2, animalnames_videotrack, output_look_ornot, output_allvectors, output_allangles, output_key_locations):
    ...
    for ianimal in np.arange(0,nanimals,1):
        ...
        names_all = ['gaze_other_angle', 'gaze_tube_angle', 'gaze_lever_angle',
        ...
        names_lowvif = ['gaze_lever_angle', 'mass_move_speed', 'gaze_angle_speed']
        ...
        names_other = ['gaze_other_angle', 'gaze_tube_angle',
                     'animal_animal_dist', 'animal_tube_dist', 'animal_lever_dist',
                     ]
        ...
        def make_design_matrix(predictors, basis_funcs):
            X = np.hstack([convolve_with_basis(v, basis_funcs) for v in predictors])
            return StandardScaler().fit_transform(X)

        # Design matrices
        X_full  = make_design_matrix(predictors_all, basis_funcs)
        X_short = make_design_matrix(predictors_lowvif, basis_funcs)
        X_other = make_design_matrix(predictors_other, basis_funcs)
        ...
        for i in range(N_BOOTSTRAPS):
            ...
            # GLM full
            X_tr, X_te, y_tr, y_te = train_test_split(X_full, Y, test_size=0.2, stratify=Y, random_state=i)
            clf_full = LogisticRegression(class_weight='balanced', max_iter=500)
            clf_full.fit(X_tr, y_tr)
            y_pred_full = clf_full.predict(X_te)
            y_score_full = clf_full.predict_proba(X_te)[:, 1]
            acc_full = accuracy_score(y_te, y_pred_full)
            auc_full = roc_auc_score(y_te, y_score_full)
            acc_full_list.append(acc_full)
            auc_full_list.append(auc_full)

            # Shortlist GLM
            X_tr_s, X_te_s, y_tr, y_te = train_test_split(X_short, Y, test_size=0.2, stratify=Y, random_state=i)
            clf_short = LogisticRegression(class_weight='balanced', max_iter=500)
            clf_short.fit(X_tr_s, y_tr)
            y_pred_short = clf_short.predict(X_te_s)
            y_score_short = clf_short.predict_proba(X_te_s)[:, 1]
            acc_short = accuracy_score(y_te, y_pred_short)
            auc_short = roc_auc_score(y_te, y_score_short)
            acc_short_list.append(acc_short)
            auc_short_list.append(auc_short)
            
            # other GLM - non shortlist
            X_tr_s, X_te_s, y_tr, y_te = train_test_split(X_other, Y, test_size=0.2, stratify=Y, random_state=i)
            clf_other = LogisticRegression(class_weight='balanced', max_iter=500)
            clf_other.fit(X_tr_s, y_tr)
            y_pred_other = clf_other.predict(X_te_s)
            y_score_other = clf_other.predict_proba(X_te_s)[:, 1]
            acc_other = accuracy_score(y_te, y_pred_other)
            auc_other = roc_auc_score(y_te, y_score_other)
            acc_other_list.append(acc_other)
            auc_other_list.append(auc_other)

        # Save to summary
        acc_df = pd.DataFrame({
            'acc_full': acc_full_list,
            'auc_full': auc_full_list,
            'acc_short': acc_short_list,
            'auc_short': auc_short_list,
            'acc_other': acc_other_list,
            'auc_other': auc_other_list
        })

        if ianimal == 0:
            glm_prediction_summary[(animal1, 'predictive_perf')] = acc_df
        elif ianimal == 1:
            glm_prediction_summary[(animal2, 'predictive_perf')] = acc_df
        
        
    return glm_prediction_summary
