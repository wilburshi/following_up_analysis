...
def tracking_video_singlecam_wholebody_withNeuron_demo(bodyparts_locs_camN,output_look_ornot,output_allvectors,output_allangles,lever_loc_both,
...
session_start_time,fps,nframes,cameraID,video_file_original,sqr_thres_tubelever,sqr_thres_face,sqr_thres_body,spike_time_data,lfp_filt_sess_aligned,
spike_channels_data,channel_to_depth): 
    ...
    video_file = "/gpfs/gibbs/pi/jadi/VideoTracker_SocialInter/3d_recontruction_analysis_self_and_coop_task_data_saved/example_videos_singlecam_wholebody_demo/"+cameraID+"/"+animal1_filename+"_"+animal2_filename+"/"+date_tgt+"_"+animal1_filename+animal2_filename+"_singlecam_wholebody_tracking_withNeuron_demo.mp4"
    ...
    gs=GridSpec(11,4) # 11 rows, 4 columns
    ...
    ax4=fig.add_subplot(gs[7:10,:]) # recording trace
    ...
    ax2.set_yticklabels('')
    ax2.set_xlabel('')
    ax2.set_ylabel('animal1',fontsize=15)
    ...
    ax3.set_yticklabels('')
    ax3.set_xlabel('')
    ax3.set_ylabel('animal2',fontsize=15)
    ...
    ax4.set_xticklabels(list(map(str,np.arange(0/fps,nframes/fps,300/fps))))
    ax4.tick_params(axis='x', labelsize=20)
    ax4.set_ylim([-65*2,0]) # all 64 channels
    ax4.set_yticklabels('')
    ax4.set_xlabel('time (s)',fontsize = 24)
    ax4.set_ylabel('')
    # ax4.set_title('neural activity trace')
            ...
            for ianimal in np.arange(0,nanimals,1):    
                ...
                for iplotframe in np.arange(0,nplotframes,1):
                    ...
                    if (ianimal == 0):
                        ...
                        if (np.isin(bhv_events_iframe,look_at_lever_framenum_plot)): 
                            ax2.plot([bhv_events_iframe,bhv_events_iframe],[0,1],'-',color = 'g')
                        if (np.isin(bhv_events_iframe,look_at_tube_framenum_plot)): 
                ...
                if (ianimal == 0):
                    for ichannel_iplot in np.arange(0,64,1):
                        ...
                        for ispike in spike_time_data_ichannel:
                            ax4.plot([ispike,ispike],[(ichannel_depth-0.7)*2,(ichannel_depth+0.7)*2],'k-')

                        #
                        # plot LFP
                        lfp_filt_ichannel = lfp_filt_sess_aligned[ichannel_iplot,iframe_min:iframe+1]
                        lfp_filt_ichannel_forplot = lfp_filt_ichannel*2*0.9 + ichannel_depth*2
                        ax4.plot(lfp_filt_ichannel_forplot,'-',color = (0.7,0.7,0.7))
