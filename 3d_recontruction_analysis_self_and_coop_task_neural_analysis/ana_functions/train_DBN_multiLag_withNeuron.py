...
########################################
def train_DBN_multiLag_create_df_only(totalsess_time, session_start_time, temp_resolu, spike_time_point, time_point_pull1, time_point_pull2, oneway_gaze1, oneway_gaze2, mutual_gaze1, mutual_gaze2):
...
# totalsess_time: the time (s) of the total session
    ...
    # organize spike time data
    spike_time_point_unique = np.unique(spike_time_point)  
    nspike_uniques = np.shape(spike_time_point_unique)[0] 
    spike_time_point_num = np.zeros((nspike_uniques,2))
    #
    for ispike_unique in np.arange(0,nspike_uniques,1):
        ispike_time_unique = spike_time_point_unique[ispike_unique]
        #
        spike_time_point_num[ispike_unique,0] = ispike_time_unique
    ...
    # spikes_threshold = np.nanmean(spike_time_point_num[spike_time_point_num[:,0]<0,1])+1*np.nanstd(spike_time_point_num[spike_time_point_num[:,0]<0,1])
    #
    # only consider time point when spike counts are higher than the baseline (the last five second of the recording,50 data point)
    spikes_outlier = np.nanmean(spike_time_point_num[:,1])+3*np.nanstd(spike_time_point_num[:,1])
    spikes_outlier = 200
    spike_time_point_num = spike_time_point_num[spike_time_point_num[:,1]<spikes_outlier,:] 
    ...
    spikes_t3 = np.zeros((total_time+3,1))
    spikes_t3[np.array(time_point_spikes_round)] = 1
    # t2 - one step back
    ...
    spikes_t2 = np.zeros((total_time+3,1))
    spikes_t2[np.array(time_point_spikes_round)+1] = 1
    # t1 - two steps back
    ...
    spikes_t1 = np.zeros((total_time+3,1))
    spikes_t1[np.array(time_point_spikes_round)+2] = 1
    # t0 - three steps back
    ...
    spikes_t0 = np.zeros((total_time+3,1))
    spikes_t0[np.array(time_point_spikes_round)+3] = 1
    
    # clean up dataframe
    data = np.concatenate((pull1_t0,pull2_t0,owgaze1_t0,owgaze2_t0,spikes_t0,pull1_t1,pull2_t1,owgaze1_t1,owgaze2_t1,spikes_t1, pull1_t2,pull2_t2,owgaze1_t2,owgaze2_t2,spikes_t2,pull1_t3,pull2_t3,owgaze1_t3,owgaze2_t3,spikes_t3),axis = 1)
    colnames = ["pull1_t0","pull2_t0","owgaze1_t0","owgaze2_t0","spikes_t0","pull1_t1","pull2_t1","owgaze1_t1","owgaze2_t1","spikes_t1", "pull1_t2","pull2_t2","owgaze1_t2","owgaze2_t2","spikes_t2","pull1_t3","pull2_t3","owgaze1_t3","owgaze2_t3","spikes_t3"]
...
########################################
def train_DBN_multiLag(totalsess_time, session_start_time, temp_resolu, time_point_pull1, time_point_pull2, oneway_gaze1, oneway_gaze2, mutual_gaze1, mutual_gaze2):
# temp_resolu: temporal resolution, the time different between each step
# e.g. temp_resolu = 0.5s, each step is 0.5s
# totalsess_time: the time (s) of the total session
...
########################################
def train_DBN_multiLag_eachtrial(totalsess_time, session_start_time, temp_resolu, time_point_pull1, time_point_pull2, oneway_gaze1, oneway_gaze2, mutual_gaze1, mutual_gaze2):
# temp_resolu: temporal resolution, the time different between each step
# e.g. temp_resolu = 0.5s, each step is 0.5s
# totalsess_time: the time (s) of the total session
